...
def prepare_data(events_df: pd.DataFrame, npc_codes_df: pd.DataFrame):
    ...
    handle missing values, drop unnecessary columns, clean the 'type' column, and add a 'duration' column.
    ...
    Returns:
    pd.DataFrame: The prepared DataFrame with missing values handled, unnecessary columns removed, and new columns added.
    """
    ...
    df_prepared['start'] = pd.to_datetime(df_prepared['start'], format='%Y-%m-%d')
    df_prepared['end'] = pd.to_datetime(df_prepared['end'], format='%Y-%m-%d')
    
    # Calculate 'duration' and insert the new column after 'end'
    df_prepared.insert(df_prepared.columns.get_loc('end') + 1, 'duration',
...
if __name__ == "__main__":
    try:
        ...
        # Print the resulting DataFrame with the new 'duration' column
        print("Data with 'duration' column:")
        print(df_prepared[['start', 'end', 'duration']].head())
